...
try:
    df = pd.read_excel('dash_visu_punkt.xlsx', sheet_name='Sheet1')
...
layout = html.Div([
    ...
    html.Div(id='box-plots-container', style={'marginBottom': '40px'}),
...
def update_bar_chart(selected_clusters):
    ...
    # allocate colors for each parameter
    indicator_colors = {
        'UTCI': 'red',
        'GWP': 'green',
        'LCC': 'blue'
    ...
    return {
        ...
        'layout': {
            ...
            'xaxis':{'title': 'Cluster', 'tickvals': list(normalized_avg_df.index), 'ticktext': [f'Cluster {i+1}' for i in range(len(normalized_avg_df.index))]},
            'yaxis': {
